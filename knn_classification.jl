...
max_lat = maximum(dropna(reports[:Lat]))
min_lat = minimum(dropna(reports[:Lat]))
max_lon = maximum(dropna(reports[:Lon]))
min_lon = minimum(dropna(reports[:Lon]))
...
reports = @transform(reports, StandardLat = standardize(:Lat, max_lat, min_lat))
reports = @transform(reports, StandardLon = standardize(:Lon, max_lon, min_lon))
...
reports = @transform(reports, End = parsedate(:End))
...
reports = @transform(reports, StandardMin = standardize_minutes(:End))
...
reports = @transform(reports, StandardDay = standardize_day(:End))
...
function nearest(lat, lon, time, k)
    ...
    sorted = @orderby(reports, :Distances)
    last(@orderby(by(DataFrame(sorted[1:k, :Description]), :x1, size), :x1_1)[:1])
end
